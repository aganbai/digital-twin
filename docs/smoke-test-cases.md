...
| Python 知识服务 | `http://localhost:8000`（LlamaIndex 语义检索） |
...
| A-02 | 教师注册完善资料 | 游客 | P0 | A-01 已登录 | 1. POST /api/auth/complete-profile<br>2. role=teacher, nickname, school, description | 返回 persona_id + 新 token | API |
| A-03 | 学生注册完善资料 | 游客 | P0 | A-01 已登录 | 1. POST /api/auth/complete-profile<br>2. role=student, nickname | 返回 persona_id + 新 token | API |
| A-04 | 已有账号登录 | 教师 | P0 | 已注册 | 1. POST /api/auth/wx-login<br>2. 使用已有 code | 返回 token + personas 列表非空 | API |
...
| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| B-01 | 创建教师分身 | 教师 | P0 | 已登录 | 1. POST /api/personas<br>2. role=teacher, nickname, school | 返回新 persona_id | API |
| B-02 | 创建学生分身 | 学生 | P0 | 已登录 | 1. POST /api/personas<br>2. role=student, nickname | 返回新 persona_id | API |
| B-03 | 查看分身列表 | 教师 | P0 | 有分身 | 1. GET /api/personas | 返回分身列表，包含 id/nickname/role | API |
| B-04 | 切换分身 | 教师 | P0 | 有多个分身 | 1. PUT /api/personas/:id/switch | 返回新 token，persona_id 已更新 | API |
| B-05 | 编辑分身信息 | 教师 | P1 | 有分身 | 1. PUT /api/personas/:id<br>2. 修改 nickname/description | 更新成功，GET 验证信息已变更 | API |
| B-06 | 停用/启用分身 | 教师 | P1 | 有分身 | 1. PUT /api/personas/:id/deactivate<br>2. PUT /api/personas/:id/activate | 分身状态正确切换 | API |
| B-07 | 分身概览页渲染 | 教师 | P1 | 有分身 | 1. 进入 persona-overview 页面 | 显示所有分身卡片 | E2E |

### 模块C：班级管理（8条）

| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| C-01 | 创建班级 | 教师 | P0 | 已登录教师分身 | 1. POST /api/classes<br>2. name, description | 返回班级 id | API |
...
| C-07 | 班级创建页UI | 教师 | P2 | 已登录 | 1. 进入 class-create 页面<br>2. 填写表单 | 表单字段完整，提交成功 | E2E |
| C-08 | 班级详情页渲染 | 教师 | P2 | 有班级 | 1. 进入 class-detail 页面 | 班级信息+成员列表正确显示 | E2E |

### 模块D：分享码与师生关系（8条）

| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| D-01 | 创建分享码 | 教师 | P0 | 有教师分身 | 1. POST /api/shares<br>2. persona_id | 返回 share_code | API |
...
| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| E-01 | 学生发送消息 | 学生 | P0 | 有师生关系 | 1. POST /api/chat<br>2. teacher_persona_id + message | 返回 AI 回复 | API |
...
| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| G-01 | 学生聊天列表 | 学生 | P0 | 有师生关系 | 1. GET /api/chat-list/student | 返回教师列表 | API |
...
| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| H-01 | 添加文本文档 | 教师 | P0 | 已登录 | 1. POST /api/documents<br>2. title + content | 文档创建成功 | API |
...
| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| K-01 | 创建课程 | 教师 | P1 | 已登录 | 1. POST /api/courses<br>2. title + description | 课程创建成功 | API |
...
| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| L-01 | 教师仪表盘API | 教师 | P0 | 有分身 | 1. GET /api/personas/:id/dashboard | 返回统计数据 | API |
...
| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| M-01 | 发现页数据加载 | 学生 | P1 | 已登录 | 1. GET /api/discover | 返回推荐+学科分类 | API |
| M-02 | 发现页搜索 | 学生 | P1 | 已登录 | 1. GET /api/discover/search?q=关键词 | 返回搜索结果 | API |
| M-03 | 分身广场 | 学生 | P1 | 有公开分身 | 1. GET /api/personas/marketplace | 返回公开分身列表 | API |
...
| ID | 用例名称 | 角色 | 优先级 | 前置条件 | 操作步骤 | 预期结果 | 验证方式 |
|----|---------|------|--------|---------|---------|---------|----------|
| R-01 | 教师端 TabBar | 教师 | P1 | 已登录 | 1. 检查底部 TabBar | 4 Tab: 工作台/学生/知识库/我的 | E2E |
| R-02 | 学生端 TabBar | 学生 | P1 | 已登录 | 1. 检查底部 TabBar | 4 Tab: 对话/历史/发现/我的 | E2E |
...
|------|--------|------|--------|--------|
| 分身 | ✅ B-01/02 | ✅ B-03 | ✅ B-05 | ✅ B-06(停用) |
| 班级 | ✅ C-01 | ✅ C-02/04 | ✅ C-06 | ✅ C-05(移除成员) |
| 分享码 | ✅ D-01 | ✅ D-04 | - | - |
| 对话 | ✅ E-01/04 | ✅ E-05 | - | - |
